<nav class="menu navbar navbar-inverse" role="navigation">
    <div class="main-menu">
        <div class="header">
            <img id="BSheaderLogo" src="http://academy.binary-studio.com/resources/logo.png" class="logo"/>
            <div>
                <div class="buttons">
                    <input id="searchBar" placeholder="Search"/>
                    <a id="userLink" class="noTextDecoration">
                        <div id="userProfile">
                            @yield('user')
                        </div>
                    </a>
                    <button id="appsBtn" class="hdr-button">
                        <img src="http://team.binary-studio.com/app/images/Thumbnails.png" class="appsLogo"/>
                    </button>
                    <button id="notificationBtn" class="hdr-button">
                        <img src="http://team.binary-studio.com/app/images/bell.png" class="appsLogo"/>
                    </button>
                </div>
                <div id="notificationCounter"></div>
                <div id="search"></div>
            </div>
        </div>
        <div id="logOutBox" class="invisible headerElements">
            <div id="userprofileBtnInBox" class="logOutButtons">
                <button id="userProfileInBoxBtn" class="userprofileAndLogoutBtn hdr-button">
                    <img src="data:image/png;base64," class="userprofileAndLogoutImg"/>
                </button>
            </div>
            <div id="logOutBtnInBox" class="logOutButtons">
                <button id="logOutButton" class="userprofileAndLogoutBtn hdr-button">
                    <img src="data:image/png;base64," class="userprofileAndLogoutImg"/>
                </button>
            </div>
        </div>
        <div id="appsBlock" class="invisible">
            <div id="appsList">
                @yield('services')
            </div>
        </div>
        <div id="notificationBlock" class="invisible">
            <ul id="notificationList">
                <li class="liNotification">
                    @yield('notifications')
                </li>
            </ul>
            <a id="readAllBtn" href="" class="noTextDecoration specialNotifBtn">
                <img width="20" height="20" src="data:image/png;base64,">
            </a>
            <a href="http://team.binary-studio.com/app/#/" class="noTextDecoration specialNotifBtn">
                <img width="20" height="20" src="data:image/png;base64,">
            </a>
            <a href="http://team.binary-studio.com/app/#/settings" class="noTextDecoration specialNotifBtn">
                <img width="20" height="20" src="data:image/png;base64,">
            </a>
        </div>
    </div>
    <div class="custom-menu">
        <button class="menu-list-control button">
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
        </button>
        <button class="language button lang">EN</button>
    </div>
</nav>